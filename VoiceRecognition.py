...
def fbank(signal, samplerate=16000, winlen=0.025, winstep=0.01,
          nfilt=26, nfft=4096, lowfreq=0, highfreq=None, preemph=0.97,
          winfunc=lambda x: numpy.ones((x,))):
...
def get_filterbanks(nfilt=20, nfft=4096, samplerate=16000, lowfreq=0, highfreq=None):
...
def split_codebook(data,  codebook, epsilon, initial_avg_dist):
    ...
    while err > epsilon:
        ...
        vecs_idxs_near_c = defaultdict(list)
...
def read_file(str):
    ...
    sample_rate, signal1 = scipy.io.wavfile.read(str)
    # print("RERSRSFFDGDGDHHFHH", sample_rate)
#    signal1 = signal1[0:int(2 * sample_rate)]  # Keep the first 2 seconds
    signal1 = signal1[0:int(2 * sample_rate)]  # Keep the first 2 seconds
    feat1 = mfcc(signal1, sample_rate, 0.025, 0.01,
                 13, 26, 2048, 0, None, 0.97, 22, True)
    return feat1
...
#
#
codebook1, codebook_abs_weights, codebook_rel_weights = generate_codebook(
    feat2, size_codebook=10, epsilon=0.00001)
...
def write_file():
    ...
    Input2 = read_file(Ip)
    codes, codebook_abs_weights, codebook_rel_weights = generate_codebook(
        Input2, size_codebook=10, epsilon=0.00001)
...
def matching_check_file(InputMFCC):
    with open("Voicerecognition.pkl", "rb") as pkl4:
        ...
        for i in data:
            ...
            print(i, matching(InputMFCC, data[i]), "FINAL MATCHES")
...
Choice = input("WHAT DO YOU WANT TO DO?\n\nEnter 1 to insert voices in our file\nEnter 2 to Check matching with other voices by inputting a new voice\nEnter 3 to check shape of file\nEnter 4 to Store a new voice to database live(Through Microphone)\nEnter 5 to check best matching voice by inputting a live new voice (Through Microphone) \n \nEnter a choice: ")
if Choice == '1':
    file_call()
# elif Choice == '2':
...
signal = numpy.frombuffer(signal, dtype=int)
# If Stereo
...
# Taking only 2 dimensions at a time. [:,5]- Takes all values of 5th dimension
